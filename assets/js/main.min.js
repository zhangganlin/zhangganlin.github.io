...
// function limitNewsHeight() {
//   const container = document.getElementById('news-container');
//   const list = document.getElementById('news-list');
//   const items = list.querySelectorAll('li');

//   // Get parameters from data attributes
//   const showCount = parseInt(container.dataset.show) || 2;      // default 2
//   const extraPadding = parseInt(container.dataset.padding) || 20; // default 20
...
//     }
//     height += extraPadding; // Add padding from data attribute
//     container.style.maxHeight = height + 'px';
//   }
// }

// // Run after page loads
// window.addEventListener('load', limitNewsHeight);

function limitNewsHeight(containerId) {
  const container = document.getElementById(containerId);
  const list = container.querySelector('ul');
  const items = list.querySelectorAll('li');

  const showCount = parseInt(container.dataset.show) || 2;

  if (items.length === 0) return;

  // If everything already fits, don't constrain.
  if (items.length <= showCount) { container.style.maxHeight = ''; return; }

  // Cut exactly at the top edge of the first hidden item => exactly `showCount`
  // entries, with no partial row visible.
  const listTop = list.getBoundingClientRect().top;
  const cutTop = items[showCount].getBoundingClientRect().top;
  container.style.maxHeight = (cutTop - listTop) + 'px';
}

// Call on page load and recompute once layout settles / on resize
function applyNewsHeight() { limitNewsHeight('news-container'); }
applyNewsHeight();
window.addEventListener('load', applyNewsHeight);
window.addEventListener('resize', applyNewsHeight);

// Add a "more"/"less" toggle to abstracts that are clamped/overflowing
...
window.addEventListener('load', setupAbstractToggles);
